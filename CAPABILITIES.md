...
- 模块：`brand`。
- 审阅的业务源码：[`db76fd87f7832e3f6e82dd8f3c3f7b42beed1d12`](https://github.com/xiafanzeng/frontmind-brand/tree/db76fd87f7832e3f6e82dd8f3c3f7b42beed1d12)。
...
| 品牌词库 | 已包含：词表展示、词库生成业务、任务处理和所属工作流。 | `module/client/dashboard/ManagedKeywordTables.tsx`；`module/server/brand-question-universe-service.ts`；`module/worker/keywords.ts`；`module/workflows/generate-brand-question-universe/` | 见下方本地检查；不据此推定真实业务通过。 | 真实词库页面与样式通过；词库数据读取和保存、供应商生成仍待验收。 |
| 企业问答 | 已包含：企业问答页面和对应业务会话入口。 | `module/client/dashboard/EnterpriseQaWorkspace.tsx`；`module/server/conversations.ts` | 见下方本地检查；不据此推定真实业务通过。 | 空问答草稿的 API 创建、列表持久化、独立读取、再次保存和删除通过。问答输入 UI 因缺少已发布知识库而阻塞；真实问答生成和执行恢复未验收。 |
...
名称修改基线完成 frozen install、typecheck、35 项 Vitest 测试、1 项工作流版本 Node 测试和完整 build。随后草稿归属与会话稳定性修复完成 typecheck、完整 build 及 34 项 focused 回归，覆盖空草稿经真实 HTTP router 保存后读取、多组件挂载、同值通知、历史任务恢复和显式启动查询。两组测试有重叠，不将其相加为测试总数。词库样式修复另完成 frozen install、typecheck、完整 build、最终前端构建，以及桌面/手机视口和搜索筛选的本地预览检查。
...
| 范围 | 已有证据与待完成项 |
|---|---|
| 目标版本页面 | 已核验 moduleSha `db76fd87f7832e3f6e82dd8f3c3f7b42beed1d12` / coreSha `54aedc504c95a6cf4ec4fdb8c12e86c90c46bb06`。知识库、品牌词库、企业问答前置页和 AI 官网四个入口、深链、刷新及通用智能体路径回首页通过。真实截图确认词库样式恢复，首页无持续加载和重复 404；无名称测试后缀、预览标记、浏览器异常或失败业务请求。 |
| 普通保存与刷新 | 真实子域名的空企业问答草稿 API 创建、列表持久化、独立读取、再次保存、删除与清理通过。问答输入 UI 因工作区没有已发布知识库而阻塞，不能记为 UI 问答流程通过。资料上传保存、节点编辑和下载仍待实测。 |
...
企业问答需先完成并发布企业知识库；当前工作区未满足此前置条件，空草稿 API 持久化通过不代表问答输入或生成通过。AI 官网的草稿设置、预览分析与部件配置按原能力提供，不计为新增完成的真实建站链路。已有 SiteOps 状态机和工作流源码不等于该环境已完成真实生成与发布。
