...
} from "../server/model-output-repair.js";
import { customerSafeKnowledgeAssetLabel } from "../server/knowledge-base-public-artifacts.js";
import {
  isKnowledgeBaseTextEvidencePath,
  KNOWLEDGE_BASE_WORKING_SET_POLICY,
} from "../server/knowledge-base-working-set-policy.js";
import {
  canonicalizeKnowledgeBaseCompanyName,
  canonicalizeKnowledgeBaseWebsite,
  KnowledgeBaseCompanyIdentityNormalizationError,
} from "../server/knowledge-base-company-identity.js";
import { canonicalKnowledgeBaseMarkdown } from "../server/knowledge-base-package-validation.js";
...
export async function canonicalWorkingSetArchive(input: {
  manifest: KnowledgeBaseWorkingSetManifest;
  files: ReadonlyMap<string, Buffer>;
}) {
  const files = new Map<string, Buffer>();
