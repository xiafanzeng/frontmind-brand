...
function compareEntryPaths(left: string, right: string) {
 ...
 */
export async function canonicalKnowledgeBaseSkillArchiveHash(archive: Buffer | Uint8Array | ArrayBuffer) {
...
export async function legacyKnowledgeBaseSkillInstructionHash(archive: Buffer | Uint8Array | ArrayBuffer) {
  const zip = await JSZip.loadAsync(archive);
