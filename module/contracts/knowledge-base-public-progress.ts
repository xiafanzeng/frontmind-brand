...
export interface KnowledgeBaseProgressDto {
  ...
  build: {
    ...
    companyName: string;
    /** Retained during edits so only the initial walkthrough publishes automatically. */
  ...
  /** Content is confirmed and can generate a ZIP for first publication or an explicit update. */
...
export interface KnowledgeBaseActiveTurnDto {
  ...
  browserUpload?: import("./knowledge-base-upload-status.js").KnowledgeBaseBrowserUpload;
  id: string;
...
export interface KnowledgeBaseObservationDto {
  runPhase?: import("./knowledge-base-upload-state.js").KnowledgeBaseRunPhase;
  execution?: GeneralExecutionDto;
  stateEpoch: number;
  generation: number;
