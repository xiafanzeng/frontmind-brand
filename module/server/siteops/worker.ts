import { parseApprovedResetUnpublishInput } from "./reset-coordinates.js";

import { createHash, randomUUID } from "node:crypto";
import { and, desc, eq, inArray, isNull, like, lt, max, or } from "drizzle-orm";
import { visualSearchOperationInputSchema } from "../../contracts/siteops-workflow.js";
import {
...
import { siteOpsQuotaStateForProviderResult } from "./quota-state.js";
import { publicSiteOpsProviderResult } from "./public-errors.js";
import {
...
import type { SiteOpsWorkerCore } from "./worker-core.js";
export function createSiteOpsWorker(core: SiteOpsWorkerCore) {
const { runWithStoredEnterpriseProjectScope, enterpriseAccountOwnerPredicate, enterpriseSiteProfileTable, enterpriseSiteProfileOwnerPredicate, enterpriseOwnerPredicate, getDb, finalizePendingTwentyFirstCredentialRevocations, runtimeErrorForLog, localAssets, messages } = core;
const { siteBuilds, siteDeployments, siteOperations, siteProjects, socialPackages, websiteStyleSampleBatches, websiteStyleSamples } = core.tables;
...
function parseSiteOpsBuildArtifactBindings(
  value: unknown,
...
function siteOpsBuildArtifactProjection(input: {
...
function exclusiveSiteOpsLiveHeadProjection(
...
function siteOpsWorkerMayClaimStatus(status: string) {
...
function siteOpsApprovedResetMayAutoRecover(
...
function siteOpsWorkerExecutionPolicy(kind: string) {
...
function terminalSiteOpsOperationProjection(
...
function siteOpsVisualOperationCoordinates(input: {
...
function siteOpsSupplementalVisualFailureMayRecover(input: {
...
function siteOpsInitialVisualSupersededMayStaySilent(input: {
  mode: "initial" | "supplemental";
  projectStatus: string;
  projectRevision: number;
  admissionRevision: number | null;
...
function unexpectedSiteOpsProviderFailure(): SiteOpsProviderResult {
...
function knownSiteOpsBuildFailure(
  error: unknown,
...
async function appendBuildTimelineEvent(
  tx: any,
  input: {
    operation: Claimed;
...
function siteOpsExternalOperationPredatesResetEpoch(input: {
  operation: Pick<
    Claimed,
...
function siteOpsDeterministicSuccessorId(
  parentId: string,
  stage: string,
) {
...
async function enqueueAutomaticDomainSuccessor(
  tx: any,
  operation: Claimed,
...
async function runSiteOpsWorkerSweep(options?: { max?: number }) {
...
function startSiteOpsWorkerScheduler(options?: { intervalMs?: number }) {
...
async function stopSiteOpsWorkerScheduler() {
  if (scheduler) clearInterval(scheduler);
  scheduler = null;
  await sweep;
}
return { parseSiteOpsBuildArtifactBindings, siteOpsBuildArtifactProjection, exclusiveSiteOpsLiveHeadProjection, siteOpsWorkerMayClaimStatus, siteOpsApprovedResetMayAutoRecover, siteOpsWorkerExecutionPolicy, terminalSiteOpsOperationProjection, siteOpsVisualOperationCoordinates, siteOpsSupplementalVisualFailureMayRecover, siteOpsInitialVisualSupersededMayStaySilent, unexpectedSiteOpsProviderFailure, knownSiteOpsBuildFailure, appendBuildTimelineEvent, siteOpsExternalOperationPredatesResetEpoch, siteOpsDeterministicSuccessorId, enqueueAutomaticDomainSuccessor, runSiteOpsWorkerSweep, startSiteOpsWorkerScheduler, stopSiteOpsWorkerScheduler };
}
