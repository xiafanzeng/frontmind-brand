import type {DashboardAgentClient,DashboardAgentClientOptions} from "./provider-ports.js";
import type {BrandSchema} from '../../schema/index.js';
import type {MySql2Database} from 'drizzle-orm/mysql2';
import type {SQL} from 'drizzle-orm';
type Port=(...args:any[])=>any;
type SiteOperation=BrandSchema['siteOperations']['$inferSelect'];
import type {ManusV2MessageEvent,ManusV2Attachment,ManusV2StructuredOutputSchema,ProviderError,CredentialMetadata} from './provider-ports.js';
type ManusV2ApiError=ProviderError;
export interface ManusProviderCore {
 tables:BrandSchema;
 credentialProfileProjection:Port;
 getSiteOpsAgentCredential:(userId:number,id?:string)=>Promise<CredentialMetadata|null>;
 createSiteOpsCoreAgentClient:(input:DashboardAgentClientOptions)=>DashboardAgentClient;
 getDb:()=>Promise<MySql2Database<any>|null>;
 assertUpstreamPromptBudget:Port;
 classifyManusV2StructuredResultEnvelope:Port;
 latestManusV2WaitingDetail:Port;
 latestManusV2TaskState:Port;
 ManusV2ApiError:new (...args:any[])=>ProviderError;
 manusV2EventOperationToken:Port;
 manusV2EventsContainOperationToken:Port;
 ...
 readSiteOpsArtifact:Port;
 siteOpsArtifactIdForIdempotency:Port;
}
...
let credentialProfileProjection:ManusProviderCore['credentialProfileProjection'];
let getSiteOpsAgentCredential:ManusProviderCore['getSiteOpsAgentCredential'];
let createSiteOpsCoreAgentClient:ManusProviderCore['createSiteOpsCoreAgentClient'];
let getDb:ManusProviderCore['getDb'];
let assertUpstreamPromptBudget:ManusProviderCore['assertUpstreamPromptBudget'];
let classifyManusV2StructuredResultEnvelope:ManusProviderCore['classifyManusV2StructuredResultEnvelope'];
let latestManusV2WaitingDetail:ManusProviderCore['latestManusV2WaitingDetail'];
let latestManusV2TaskState:ManusProviderCore['latestManusV2TaskState'];
let ManusV2ApiError:ManusProviderCore['ManusV2ApiError'];
let manusV2EventOperationToken:ManusProviderCore['manusV2EventOperationToken'];
let manusV2EventsContainOperationToken:ManusProviderCore['manusV2EventsContainOperationToken'];
let orderManusV2EventsByProviderRank:ManusProviderCore['orderManusV2EventsByProviderRank'];
let readKnowledgeSnapshotArchive:ManusProviderCore['readKnowledgeSnapshotArchive'];
let persistSiteOpsArtifact:ManusProviderCore['persistSiteOpsArtifact'];
let readSiteOpsArtifact:ManusProviderCore['readSiteOpsArtifact'];
let siteOpsArtifactIdForIdempotency:ManusProviderCore['siteOpsArtifactIdForIdempotency'];
export function configureManusProviderCore(core:ManusProviderCore){
 ({knowledgeBaseSnapshots,siteBuildInputAssets,siteBuilds,siteOperations,siteProjects,socialPackages,websiteStyleSampleBatches,websiteStyleSamples}=core.tables);
 ({credentialProfileProjection,getSiteOpsAgentCredential,createSiteOpsCoreAgentClient,getDb,assertUpstreamPromptBudget,classifyManusV2StructuredResultEnvelope,latestManusV2WaitingDetail,latestManusV2TaskState,ManusV2ApiError,manusV2EventOperationToken,manusV2EventsContainOperationToken,orderManusV2EventsByProviderRank,readKnowledgeSnapshotArchive,persistSiteOpsArtifact,readSiteOpsArtifact,siteOpsArtifactIdForIdempotency}=core);
}
...
} from "../../contracts/siteops-content-plan.js";
import { createHostOwnedSiteDesignResultV2 } from "../../contracts/siteops-host-design.js";
import { managedAgentProfileSchema } from "@frontmind/module-contracts/agent-profile";
import {
  canonicalJson,
...
import { readSelectedOfficialLogoFromKnowledgeArchive } from "./knowledge-brand-asset.js";
import {
...
} from "./knowledge-content-media.js";
import {
...
} from "./manus-wire-contract.js";
import {
  resolveSiteOpsWireOutput,
  SITEOPS_WIRE_OUTPUT_FILES,
  SiteOpsWireOutputResolutionError,
  type SiteOpsWireOutputPhase,
} from "./manus-wire-output-resolver.js";
import {
  canonicalizeSiteContentDraft,
  canonicalPreviewToGeneratedContent,
} from "./site-content-draft.js";
import { applySiteContentPatchV1Resilient } from "./site-content-patch.js";
import { terminalTaskState } from "./task-terminal-state.js";
import {
...
} from "./native-react-build-runtime.js";
import {
...
} from "./build-artifact-checkpoint.js";

export { terminalTaskState } from "./task-terminal-state.js";
...
type ManusProviderDependencies = {
  getDb?: typeof getDb;
  getCredential?: typeof getSiteOpsAgentCredential;
...
function workflowRoots(workflow: ReturnType<typeof siteOpsWorkflowForVersion>) {
  ...
  return [
    ...
    workflow.frontMindVersion === SITEOPS_WORKFLOW.frontMindVersion
      ...
      : []),
    path.resolve(process.cwd(), "../../modules/brand/workflows", directory),
...
export async function loadVerifiedSiteOpsSocialWorkflowPackage(
  ...
  const loading = (async () => {
    const candidates = [
      ...
      path.resolve(process.cwd(), "../../modules/brand/workflows", definition.directory),
...
export function createManusSiteOpsProviderHandler(
  ...
  const getCredential =
    dependencies.getCredential ?? getSiteOpsAgentCredential;
  const createClient =
    dependencies.createClient ??
    ((input) =>
      createSiteOpsCoreAgentClient({
        ...input,
