...
export interface KnowledgeUploadEvidenceCore { assetRoot():string; knowledgeBaseBuilds: BrandSchema["knowledgeBaseBuilds"]; getDb(): Promise<MySql2Database<any> | null>; enterpriseOwnerPredicate(table:any,userId:number):SQL; }
export function createKnowledgeBaseUploadEvidenceLifecycle(core: KnowledgeUploadEvidenceCore) {
const { knowledgeBaseBuilds, getDb, enterpriseOwnerPredicate } = core;
...
const DEFAULT_KNOWLEDGE_BASE_UPLOAD_EVIDENCE_SWEEP_LIMIT = 50;
const KNOWLEDGE_BASE_UPLOAD_EVIDENCE_ORPHAN_GRACE_MS =
  24 * 60 * 60 * 1_000;
 ...
 */
function knowledgeBaseUploadEvidenceStorageKey(
...
function optionalKnowledgeBaseUploadEvidenceStorageKey(
  input: KnowledgeBaseUploadEvidenceCoordinate,
) {
  try {
...
function parseKnowledgeBaseUploadEvidenceStorageKey(
  storageKey: string,
 ...
 */
async function removeKnowledgeBaseUploadEvidenceIfOrphaned(input: {
  storageKey: string;
 ...
 */
async function sweepOrphanedKnowledgeBaseUploadEvidence(input?: {
  db?: KnowledgeBaseLifecycleDb;
  assetRoot?: string;
  limit?: number;
  cursor?: string;
...
return { DEFAULT_KNOWLEDGE_BASE_UPLOAD_EVIDENCE_SWEEP_LIMIT, KNOWLEDGE_BASE_UPLOAD_EVIDENCE_ORPHAN_GRACE_MS, knowledgeBaseUploadEvidenceStorageKey, optionalKnowledgeBaseUploadEvidenceStorageKey, parseKnowledgeBaseUploadEvidenceStorageKey, removeKnowledgeBaseUploadEvidenceIfOrphaned, sweepOrphanedKnowledgeBaseUploadEvidence };
}
