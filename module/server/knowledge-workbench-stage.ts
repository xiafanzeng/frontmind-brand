...
export interface KnowledgeWorkbenchStageCore { conversationTurns:CoreSqlTable<ConversationTurn>; enterpriseOwnerPredicate(table:any,userId:number):SQL; }
export function createKnowledgeWorkbenchStages(core: KnowledgeWorkbenchStageCore) {
const { conversationTurns, enterpriseOwnerPredicate } = core;


function knowledgeWorkbenchRecord(value: unknown): Record<string, any> {
...
async function knowledgeWorkbenchStart(
  tx: any,
...
function knowledgeWorkbenchStage(
  build: Pick<
    KnowledgeBaseBuild,
...
async function knowledgeWorkbenchEditingAllowed(
  tx: any,
  build: KnowledgeBaseBuild,
) {
  return (
...
return { knowledgeWorkbenchRecord, knowledgeWorkbenchStart, knowledgeWorkbenchStage, knowledgeWorkbenchEditingAllowed };
}
