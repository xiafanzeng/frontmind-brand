...
import type {BrandKeywordEvent as ManusV2MessageEvent} from "./brand-keyword-ports.js";
export function createBrandQuestionExecutionProjector(projectGeneralExecution:(taskId:string,events:any[])=>import("@frontmind/module-contracts/execution").GeneralExecutionDto) {
/** Actual saved workflow and provider evidence; no prompt, result body or private thought. */
function projectBrandQuestionExecution(input: {
  taskId: string;
  status: string;
  createdAt: number;
  updatedAt: number;
...
return projectBrandQuestionExecution;
}
