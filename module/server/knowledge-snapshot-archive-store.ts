...
export interface KnowledgeSnapshotArchiveCore { assetRoot(): string; installImmutableFileAtomically(input: {target:string;buffer:Buffer}): Promise<"installed"|"exists">; }
export function createKnowledgeSnapshotArchiveStore(core: KnowledgeSnapshotArchiveCore) {
...
function knowledgeSnapshotArchiveStorageKey(
  userId: number,
  snapshotId: string,
) {
...
function knowledgeArchiveContentDisposition(filename: string) {
  const safe =
    path
...
return { knowledgeSnapshotArchiveStorageKey, hasZipMagic, persistKnowledgeSnapshotArchive, readKnowledgeSnapshotArchive, isKnowledgeSnapshotArchiveAvailable, removeKnowledgeSnapshotArchive, knowledgeArchiveContentDisposition };
}
