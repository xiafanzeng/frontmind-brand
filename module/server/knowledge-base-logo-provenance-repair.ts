...
import type { KnowledgeBaseClientAttachmentManifestItem } from "./knowledge-base-client-attachment-manifest.js";
import type {
...
import { knowledgeBaseConversationStorageId } from "./knowledge-base-turn-service.js";
...
export interface KnowledgeServiceCore extends Pick<ReturnType<typeof createKnowledgeBuildArtifactStore>,"readKnowledgeBuildArtifact"> {knowledgeBaseBuildNodes:BrandSchema["knowledgeBaseBuildNodes"];knowledgeBaseBuilds:BrandSchema["knowledgeBaseBuilds"];conversationTurns:CoreSqlTable<ConversationTurn>; getDb():Promise<MySql2Database<any>|null>; enterpriseOwnerPredicate(table:any,userId:number):SQL;readStoredPresalesFile(id:string):Promise<{filename:string;mimeType:string;recordedSizeBytes:number|null;sizeBytes:number;sha256:string|null;createReadStream():Readable}|null>;}
let enterpriseOwnerPredicate: KnowledgeServiceCore["enterpriseOwnerPredicate"];
let conversationTurns: KnowledgeServiceCore["conversationTurns"];
let knowledgeBaseBuildNodes: KnowledgeServiceCore["knowledgeBaseBuildNodes"];
let knowledgeBaseBuilds: KnowledgeServiceCore["knowledgeBaseBuilds"];
let getDb: KnowledgeServiceCore["getDb"];
let readKnowledgeBuildArtifact: KnowledgeServiceCore["readKnowledgeBuildArtifact"];
let readStoredPresalesFile: KnowledgeServiceCore["readStoredPresalesFile"];
export function configureKnowledgeLogoRepair(core: KnowledgeServiceCore) { ({ enterpriseOwnerPredicate, conversationTurns, knowledgeBaseBuildNodes, knowledgeBaseBuilds, getDb, readKnowledgeBuildArtifact, readStoredPresalesFile } = core); }
