...
export interface KnowledgeLocalSourceCore { assetRoot(): string; installImmutableFileAtomically(input: {target:string;buffer:Buffer}): Promise<"installed"|"exists">; }
export function createKnowledgeBaseLocalSourceStore(core: KnowledgeLocalSourceCore) {
const { installImmutableFileAtomically } = core;
 ...
 */
async function persistKnowledgeBaseGeneratedSource(bytes: Buffer) {
  assertBytes(bytes);
 ...
 */
async function persistKnowledgeBaseSkillArchive(bytes: Buffer) {
  assertBytes(bytes);
  const contentSha256 = sha256(bytes);
...
async function persistKnowledgeBaseBuildSource(input: {
  userId: number;
  buildId: string;
  generation: number;
  bytes: Buffer;
...
async function readKnowledgeBaseLocalSource(input: {
  storageKey: string;
...
async function* streamKnowledgeBaseLocalSource(input: { storageKey: string; contentSha256: string; sizeBytes: number }) {
...
return { persistKnowledgeBaseGeneratedSource, persistKnowledgeBaseSkillArchive, persistKnowledgeBaseBuildSource, readKnowledgeBaseLocalSource, streamKnowledgeBaseLocalSource };
}
