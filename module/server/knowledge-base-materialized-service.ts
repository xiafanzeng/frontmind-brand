...
import { uploadMetadataVersion } from "./knowledge-base-upload-state.js";
import {
...
import { knowledgeBaseObservationConversationStorageId } from "./knowledge-base-progress-service.js";
import {
  canonicalizeKnowledgeBaseCompanyName,
  canonicalizeKnowledgeBaseWebsite,
} from "./knowledge-base-company-identity.js";
import {
...
import { knowledgeBaseInitialUploadLedgerFingerprint, retainInitialKnowledgeBaseImages } from "./knowledge-base-initial-images.js";
import type {BrandSchema} from "../schema/index.js";
import type {CoreSqlTable} from "../contracts/sql-table.js";
import type {ConversationTurn} from "../contracts/core-records.js";
import type {MySql2Database} from "drizzle-orm/mysql2";
import type {SQL} from "drizzle-orm";
import type {createKnowledgeBuildArtifactStore} from "./knowledge-build-artifact-store.js";
...
export interface KnowledgeMaterializationCore extends Pick<ReturnType<typeof createKnowledgeBuildArtifactStore>, "knowledgeBuildArtifactCandidateStorageKey"|"persistKnowledgeBuildArtifact"|"readKnowledgeBuildArtifact"|"removeKnowledgeBuildArtifact"|"removeStagedKnowledgeBuildArtifact">,Pick<ReturnType<typeof createKnowledgeBaseMaterializedAssets>, "knowledgeBaseWorkingSetLeafLocalUrls">,Pick<ReturnType<typeof createKnowledgeWorkbenchStages>, "knowledgeWorkbenchEditingAllowed">,Pick<ReturnType<typeof createKnowledgeBaseAuthoritativeMessages>, "knowledgeBasePresentationKey">,Pick<ReturnType<typeof createKnowledgeBaseConversationMessages>, "persistKnowledgeBaseCompletionInTransaction"|"persistKnowledgeBasePresentationInTransaction"|"persistKnowledgeBaseUserMessageInTransaction">,Pick<ReturnType<typeof createKnowledgeBaseLocalSourceStore>, "persistKnowledgeBaseBuildSource"|"readKnowledgeBaseLocalSource"> {
 getDb():Promise<MySql2Database<any>|null>; enterpriseOwnerPredicate(table:any,userId:number):SQL;
 knowledgeBaseBuildNodes:BrandSchema["knowledgeBaseBuildNodes"];knowledgeBaseBuilds:BrandSchema["knowledgeBaseBuilds"];knowledgeBaseExecutions:BrandSchema["knowledgeBaseExecutions"];knowledgeBaseWorkingSets:BrandSchema["knowledgeBaseWorkingSets"];
 conversationTurns:CoreSqlTable<ConversationTurn>;
 enterpriseResetStateTable():BrandSchema["knowledgeBaseResetStates"];enterpriseResetStateOwnerPredicate(userId:number):SQL;
 }
let knowledgeWorkbenchEditingAllowed: KnowledgeMaterializationCore["knowledgeWorkbenchEditingAllowed"];
let enterpriseResetStateTable: KnowledgeMaterializationCore["enterpriseResetStateTable"];
let enterpriseResetStateOwnerPredicate: KnowledgeMaterializationCore["enterpriseResetStateOwnerPredicate"];
let enterpriseOwnerPredicate: KnowledgeMaterializationCore["enterpriseOwnerPredicate"];
let conversationTurns: KnowledgeMaterializationCore["conversationTurns"];
let knowledgeBaseBuildNodes: KnowledgeMaterializationCore["knowledgeBaseBuildNodes"];
let knowledgeBaseBuilds: KnowledgeMaterializationCore["knowledgeBaseBuilds"];
let knowledgeBaseExecutions: KnowledgeMaterializationCore["knowledgeBaseExecutions"];
let knowledgeBaseWorkingSets: KnowledgeMaterializationCore["knowledgeBaseWorkingSets"];
let getDb: KnowledgeMaterializationCore["getDb"];
let knowledgeBasePresentationKey: KnowledgeMaterializationCore["knowledgeBasePresentationKey"];
let persistKnowledgeBaseCompletionInTransaction: KnowledgeMaterializationCore["persistKnowledgeBaseCompletionInTransaction"];
let persistKnowledgeBasePresentationInTransaction: KnowledgeMaterializationCore["persistKnowledgeBasePresentationInTransaction"];
let persistKnowledgeBaseUserMessageInTransaction: KnowledgeMaterializationCore["persistKnowledgeBaseUserMessageInTransaction"];
let knowledgeBaseWorkingSetLeafLocalUrls: KnowledgeMaterializationCore["knowledgeBaseWorkingSetLeafLocalUrls"];
let knowledgeBuildArtifactCandidateStorageKey: KnowledgeMaterializationCore["knowledgeBuildArtifactCandidateStorageKey"];
let persistKnowledgeBuildArtifact: KnowledgeMaterializationCore["persistKnowledgeBuildArtifact"];
let readKnowledgeBuildArtifact: KnowledgeMaterializationCore["readKnowledgeBuildArtifact"];
let removeKnowledgeBuildArtifact: KnowledgeMaterializationCore["removeKnowledgeBuildArtifact"];
let removeStagedKnowledgeBuildArtifact: KnowledgeMaterializationCore["removeStagedKnowledgeBuildArtifact"];
let persistKnowledgeBaseBuildSource: KnowledgeMaterializationCore["persistKnowledgeBaseBuildSource"];
let readKnowledgeBaseLocalSource: KnowledgeMaterializationCore["readKnowledgeBaseLocalSource"];
export function configureKnowledgeBaseMaterialization(core: KnowledgeMaterializationCore) { ({ knowledgeWorkbenchEditingAllowed, enterpriseResetStateTable, enterpriseResetStateOwnerPredicate, enterpriseOwnerPredicate, conversationTurns, knowledgeBaseBuildNodes, knowledgeBaseBuilds, knowledgeBaseExecutions, knowledgeBaseWorkingSets, getDb, knowledgeBasePresentationKey, persistKnowledgeBaseCompletionInTransaction, persistKnowledgeBasePresentationInTransaction, persistKnowledgeBaseUserMessageInTransaction, knowledgeBaseWorkingSetLeafLocalUrls, knowledgeBuildArtifactCandidateStorageKey, persistKnowledgeBuildArtifact, readKnowledgeBuildArtifact, removeKnowledgeBuildArtifact, removeStagedKnowledgeBuildArtifact, persistKnowledgeBaseBuildSource, readKnowledgeBaseLocalSource } = core); }
...
export async function activateInitialKnowledgeBaseWorkingSet(input: {
  ...
  const providerWorkingSet = await validateKnowledgeBaseWorkingSetArchive(
    input.archiveBytes,
  ...
  if (input.normalization) {
    assertAcceptedNormalizationMatchesWorkingSet({
      ...
      validated: providerWorkingSet,
    });
  }
  const normalization = input.normalization;
  // Read the current start turn before decoding uploads or building its ZIP.
  // The same ownership and frozen ledger are checked again under the write lock.
  const [initialBuildRows, initialTurnRows] = await Promise.all([
    db.select().from(knowledgeBaseBuilds).where(and(
      eq(knowledgeBaseBuilds.id, input.buildId),
      enterpriseOwnerPredicate(knowledgeBaseBuilds, input.userId),
    )).limit(1),
    db.select().from(conversationTurns).where(and(
  ...
  const initialBuild = initialBuildRows[0];
  const initialTurn = initialTurnRows[0];
  if (!initialBuild || initialBuild.generation !== input.generation || initialBuild.activeTurnId !== input.turnId || initialBuild.contentVersion !== 0 || initialBuild.activeWorkingSetId || !initialTurn || initialTurn.operationType !== "start" || initialTurn.operationKey !== input.operationKey || initialTurn.upstreamTaskId !== input.providerTaskId || !["queued", "running"].includes(initialTurn.status) || input.initialBundleExpectation.expectedUploadsRead !== Number(record(initialTurn.metadata)?.userAttachmentCount)) {
    fail("STALE_COORDINATES", "初始执行已失去当前构建所有权");
  }
  const uploadLedgerFingerprint = knowledgeBaseInitialUploadLedgerFingerprint(initialTurn);
  const validated = await retainInitialKnowledgeBaseImages({ workingSet: providerWorkingSet, turn: initialTurn });
  ...
  return db.transaction(async (tx: any) => {
    ...
    if (
      ...
      knowledgeBaseInitialUploadLedgerFingerprint(turn) !== uploadLedgerFingerprint
    ) {
      fail("STALE_COORDINATES", "初始执行已失去当前构建所有权");
    }
...
export async function confirmMaterializedKnowledgeBaseNode(
  ...
  return db.transaction(async (tx: any) => {
    ...
    return {
      ...
      firstPublication: !next && !build.publishedAt && !build.publishedSnapshotId,
      generation: build.generation,
      buildId: build.id,
      revision: nextRevision,
      stateEpoch: build.stateEpoch + 1,
