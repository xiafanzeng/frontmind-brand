...
import type { KnowledgeBaseLocalUploadCoordinate } from "../contracts/knowledge-base-local-upload.js";
import {
  normalizeKnowledgeBaseClientAttachmentManifest,
  type KnowledgeBaseClientAttachmentManifestItem,
} from "./knowledge-base-client-attachment-manifest.js";
...
} from "./knowledge-base-deferred-attachment-stage-policy.js";
import {
...
export interface KnowledgeServiceCore {localAssets:CoreSqlTable<LocalAsset>; getDb():Promise<MySql2Database<any>|null>; enterpriseAccountOwnerPredicate(table:any,userId:number):SQL;readStoredPresalesFile(id:string):Promise<{filename:string;mimeType:string;sizeBytes:number;sha256:string|null;createReadStream():Readable}|null>;}
let enterpriseAccountOwnerPredicate: KnowledgeServiceCore["enterpriseAccountOwnerPredicate"];
let localAssets: KnowledgeServiceCore["localAssets"];
let getDb: KnowledgeServiceCore["getDb"];
let readStoredPresalesFile: KnowledgeServiceCore["readStoredPresalesFile"];
export function configureKnowledgeDeferredUploadRecovery(core: KnowledgeServiceCore) { ({ enterpriseAccountOwnerPredicate, localAssets, getDb, readStoredPresalesFile } = core); }
