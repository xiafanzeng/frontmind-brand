...
import {z} from "zod";
import {generalExecutionActivity} from "@frontmind/module-contracts/execution";
import {assertUpstreamPromptBudget} from "@frontmind/module-contracts/upstream-prompt-budget";
import {brandQuestionUniverseStartInputSchema,BrandQuestionUniverseValidationError,type BrandQuestionUniversePayload} from "../contracts/brand-question-universe.js";
import type {BrandKeywordPorts,BrandKeywordActor as AuthenticatedUser,BrandKeywordCredential as DecryptedCredential,BrandKeywordEvent as ManusV2MessageEvent} from "./brand-keyword-ports.js";
import {
...
} from "./brand-question-universe-runtime.js";

export function createBrandQuestionUniverseService(core:BrandKeywordPorts) {
const {projectBrandQuestionExecution, lockCustomerProjectBusinessWrite, currentEnterpriseProjectId, enterpriseWorkspaceUserId, getEnterpriseProjectScope, runWithStoredEnterpriseProjectScope, enterpriseAccountOwnerPredicate, enterpriseDashboardTable, enterpriseDashboardOwnerPredicate, createCredentialAgentClient, agentEvents, agentOperations, agentTasks, artifacts, localAssets, dashboardPayloadSchema, getApiCredentialStatus, getDecryptedCredentialForAccountById, getDecryptedCredentialForUser, recordUpstreamResource, getLatestAuthenticatedKnowledgeSnapshot, DashboardRevisionConflictError, getDashboardWorkspace, getKnowledgeSnapshotById, updateDashboardWorkspace, getDb, classifyManusV2StructuredResultEnvelope, latestManusV2TaskState, ManusV2ApiError, manusV2EventOperationToken, manusV2EventsContainOperationToken, orderManusV2EventsByProviderRank, assertServiceCapability, getUpstreamBaseUrl, sealLocalAssetStorageIdentity, readStoredPresalesFile, removeStoredPresalesFile, stagePresalesFileContent}=core;
const BRAND_QUESTION_UNIVERSE_OPERATION_TYPE = "brand_question_universe";
const BRAND_QUESTION_UNIVERSE_CONTRACT_REVISION = 1;
const BRAND_QUESTION_UNIVERSE_MAX_REPAIRS = 3;
...
function brandQuestionUniverseStatusFencesStart(
...
function brandQuestionUniverseFirstDispatchAction(input: {
...
function brandQuestionUniverseCreateFailureDisposition(input: {
...
class BrandQuestionUniverseServiceError extends Error {
  constructor(
...
type BrandQuestionUniverseKnowledgeReadiness =
 ...
 */
function brandQuestionUniverseKnowledgeReadiness(
...
function brandQuestionUniversePreparationServiceError(
...
function brandQuestionUniverseFrozenRequestHash(
  context: Pick<
    OperationContext,
...
function assertCustomer(actor: AuthenticatedUser) {core.assertActorAllowed(actor);}
...
function brandQuestionUniverseReplayMatches(input: {
  operation: Pick<
...
async function rebuildReservedArchives(
  ...
  const [upstream, adapter, knowledge] = await Promise.all([
    loadBrandQuestionUniverseUpstreamArchive(context.inputHashes.upstream),
    buildBrandQuestionUniverseAdapterArchive(context.inputHashes.adapter),
...
async function reserveOperation(input: {
  ...
  const reservation = await db.transaction(async (tx: any) => {
    ...
    await core.lockAccountRecord(tx,enterpriseWorkspaceUserId(input.actor.id));
    const existing = (
      await tx
        .select({ operation: agentOperations, task: agentTasks })
        .from(agentOperations)
...
async function cachedProviderEvents(taskId: string) {
  ...
  return (rows as Array<typeof agentEvents.$inferSelect>).flatMap<ManusV2MessageEvent>((row, index) => {
...
function projectBrandQuestionUniversePublicOperation(input: {
  status: OperationStatus;
...
function brandQuestionUniverseCanRecheckFailedResult(
...
function brandQuestionUniverseTerminalResultAction(
  state: string,
...
async function observeBrandQuestionUniverse(actor: AuthenticatedUser) {
...
async function startBrandQuestionUniverse(input: {
  actor: AuthenticatedUser;
  value: z.infer<typeof brandQuestionUniverseStartInputSchema>;
}) {
 ...
 */
async function runBrandQuestionUniverseWorkerSweep(options?: {
  max?: number;
}) {
  const db = await getDb();
...
return {BRAND_QUESTION_UNIVERSE_OPERATION_TYPE, BRAND_QUESTION_UNIVERSE_CONTRACT_REVISION, BRAND_QUESTION_UNIVERSE_MAX_REPAIRS, brandQuestionUniverseStatusFencesStart, brandQuestionUniverseFirstDispatchAction, brandQuestionUniverseCreateFailureDisposition, BrandQuestionUniverseServiceError, brandQuestionUniverseKnowledgeReadiness, brandQuestionUniversePreparationServiceError, operationIdempotencyKeyHash, brandQuestionUniverseFrozenRequestHash, persistImmutableResultArtifact, brandQuestionUniverseReplayMatches, structuredRoundResult, projectBrandQuestionUniversePublicOperation, brandQuestionUniverseCanRecheckFailedResult, brandQuestionUniverseTerminalResultAction, observeBrandQuestionUniverse, startBrandQuestionUniverse, runBrandQuestionUniverseWorkerSweep};
}
