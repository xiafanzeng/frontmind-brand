...
export interface KnowledgeSourceLifecycleCore { assetRoot():string; knowledgeBaseBuilds: BrandSchema["knowledgeBaseBuilds"]; getDb(): Promise<MySql2Database<any> | null>; enterpriseOwnerPredicate(table:any,userId:number):SQL; installImmutableFileAtomically(input:{target:string;buffer:Buffer}):Promise<"installed"|"exists">; }
export function createKnowledgeBaseSourceLifecycle(core: KnowledgeSourceLifecycleCore) {
const { knowledgeBaseBuilds, getDb, enterpriseOwnerPredicate, installImmutableFileAtomically } = core;
...
const KNOWLEDGE_BASE_BUILD_SOURCE_RETENTION_MS =
  30 * 24 * 60 * 60 * 1_000;
...
async function markKnowledgeBaseBuildSourceGenerationTerminal(input: {
  userId: number;
  buildId: string;
  generation: number;
  reason: KnowledgeBaseBuildSourceTerminalReason;
 ...
 */
async function markKnowledgeBaseBuildSourcesTerminal(input: {
  userId: number;
  buildId: string;
  reason: KnowledgeBaseBuildSourceTerminalReason;
  terminalAt: Date;
 ...
 */
async function sweepKnowledgeBaseBuildSources(
  input: {
    now?: Date;
    retentionMs?: number;
    limit?: number;
...
return { KNOWLEDGE_BASE_BUILD_SOURCE_RETENTION_MS, markKnowledgeBaseBuildSourceGenerationTerminal, markKnowledgeBaseBuildSourcesTerminal, sweepKnowledgeBaseBuildSources };
}
