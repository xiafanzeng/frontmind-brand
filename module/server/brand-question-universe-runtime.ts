...
type DashboardPayload = {keywordTables:Array<{id:string;title:string;description?:string|null;columns:string[];rows:string[][]}>};
import {
...
} from "../contracts/brand-question-universe.js";
import {
...
const rootCandidates = [
  path.resolve(import.meta.dirname, "../workflows/generate-brand-question-universe"),
  path.resolve(process.cwd(), "modules/brand/workflows/generate-brand-question-universe"),
  path.resolve(
    process.cwd(),
    "private-workflows",
...
type BuiltKeywordManifest={schemaVersion:1;version:string;upstream:{filename:string;sha256:string};adapter:{filename:string;sha256:string}};
function retainedKeywordRoot(){const root=process.env.FRONTMIND_DASHBOARD_ASSET_DIR;return root?path.join(root,"workflows","brand-keywords"):null;}
async function builtKeywordArchive(kind:"upstream"|"adapter",expectedHash?:string) {
 if(expectedHash&&!/^[a-f0-9]{64}$/.test(expectedHash))throw new Error("BRAND_QUESTION_UNIVERSE_WORKFLOW_HASH_INVALID");
 const retained=retainedKeywordRoot();
 if(expectedHash&&retained){try{const bytes=await fs.readFile(path.join(retained,`${expectedHash}.zip`));if(createHash("sha256").update(bytes).digest("hex")!==expectedHash)throw new Error("BRAND_QUESTION_UNIVERSE_RETAINED_HASH_MISMATCH");return {bytes,contentHash:expectedHash,version:expectedHash.slice(0,12)};}catch(error){if((error as NodeJS.ErrnoException).code!=="ENOENT")throw error;}}
 const root=await findWorkflowRoot();let manifest:BuiltKeywordManifest;
 try{manifest=JSON.parse(await fs.readFile(path.join(root,"current.json"),"utf8"));}catch(error){if((error as NodeJS.ErrnoException).code==="ENOENT")return null;throw error;}
 const entry=manifest[kind];if(manifest.schemaVersion!==1||!/^[a-f0-9]{64}$/.test(entry.sha256)||!/^brand-keywords(?:-adapter)?-[a-f0-9]{64}\.zip$/.test(entry.filename))throw new Error("BRAND_QUESTION_UNIVERSE_BUILD_MANIFEST_INVALID");
 if(expectedHash&&entry.sha256!==expectedHash)return null;
 const bytes=await fs.readFile(path.join(root,"archives",entry.filename));if(createHash("sha256").update(bytes).digest("hex")!==entry.sha256)throw new Error("BRAND_QUESTION_UNIVERSE_BUILD_HASH_MISMATCH");
 if(retained){await fs.mkdir(retained,{recursive:true});const temporary=path.join(retained,`${entry.sha256}.${randomUUID()}.tmp`);await fs.writeFile(temporary,bytes);await fs.rename(temporary,path.join(retained,`${entry.sha256}.zip`));}
 return {bytes,contentHash:entry.sha256,version:manifest.version};
}

export async function loadBrandQuestionUniverseUpstreamArchive(expectedHash?:string) {
  const built=await builtKeywordArchive("upstream",expectedHash);if(built)return built;
  if(expectedHash&&expectedHash!==BRAND_QUESTION_UNIVERSE_UPSTREAM_SHA256)throw new Error("BRAND_QUESTION_UNIVERSE_FROZEN_WORKFLOW_UNAVAILABLE");
...
export async function buildBrandQuestionUniverseAdapterArchive(expectedHash?:string) {
  const built=await builtKeywordArchive("adapter",expectedHash);if(built)return built;
  if(expectedHash){const root=await findWorkflowRoot();try{const legacy=JSON.parse(await fs.readFile(path.join(root,"upstream/adapter-legacy.json"),"utf8"));if(legacy.contentHash===expectedHash){const bytes=await fs.readFile(path.join(root,"upstream/frontmind-adapter-legacy.zip"));if(createHash("sha256").update(bytes).digest("hex")!==legacy.sha256)throw new Error("BRAND_QUESTION_UNIVERSE_LEGACY_ADAPTER_HASH_MISMATCH");const archive=await JSZip.loadAsync(bytes,{checkCRC32:true});const manifest=JSON.parse(await archive.file("MANIFEST.json")!.async("string"));if(manifest.contentHash!==expectedHash)throw new Error("BRAND_QUESTION_UNIVERSE_LEGACY_ADAPTER_HASH_MISMATCH");return {bytes,contentHash:expectedHash,version:String(legacy.version)};}}catch(error){if((error as NodeJS.ErrnoException).code!=="ENOENT")throw error;}}

  if (adapterArchiveCache) {if(expectedHash&&adapterArchiveCache.contentHash!==expectedHash)throw new Error("BRAND_QUESTION_UNIVERSE_FROZEN_ADAPTER_UNAVAILABLE");return adapterArchiveCache;}
  const root = await findWorkflowRoot();
  ...
  if(expectedHash&&adapterArchiveCache.contentHash!==expectedHash)throw new Error("BRAND_QUESTION_UNIVERSE_FROZEN_ADAPTER_UNAVAILABLE");
