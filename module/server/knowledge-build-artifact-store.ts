import { MAX_KNOWLEDGE_SNAPSHOT_ARCHIVE_BYTES } from "@frontmind/module-brand/server/knowledge-snapshot-archive-store";
import { createHash } from "node:crypto";
import { mkdir, readFile, readdir, stat, unlink } from "node:fs/promises";
import path from "node:path";

import sharp, { type Metadata } from "sharp";
import type {KnowledgeLocalSourceCore} from "./knowledge-base-local-source-store.js";
import type {createKnowledgeSnapshotArchiveStore} from "./knowledge-snapshot-archive-store.js";
...
export interface KnowledgeBuildArtifactCore extends KnowledgeLocalSourceCore, Pick<ReturnType<typeof createKnowledgeSnapshotArchiveStore>,"hasZipMagic"> {}
export function createKnowledgeBuildArtifactStore(core: KnowledgeBuildArtifactCore) {
const { installImmutableFileAtomically, hasZipMagic } = core;
...
function knowledgeBuildArtifactStorageKey(input: {
  userId: number;
  buildId: string;
  generation: number;
  kind: KnowledgeBuildArtifactKind;
 ...
 */
function knowledgeBuildArtifactLocalPackageStorageKey(input: {
  userId: number;
  buildId: string;
  generation: number;
  revision: number;
 ...
 */
function knowledgeBuildArtifactCandidateStorageKey(input: {
  userId: number;
  buildId: string;
  generation: number;
  turnId: string;
...
function knowledgeBuildArtifactStorageKeyBelongsTo(input: {
  storageKey: string;
  userId: number;
  buildId: string;
  generation: number;
...
async function persistKnowledgeBuildArtifact(input: {
  userId: number;
  buildId: string;
  generation: number;
  kind: KnowledgeBuildArtifactKind;
...
async function stageKnowledgeBuildArtifact(input: {
  userId: number;
  buildId: string;
  generation: number;
  turnId: string;
...
async function readKnowledgeBuildArtifact(input: {
  userId: number;
  buildId: string;
  generation: number;
  kind: KnowledgeBuildArtifactKind;
...
async function removeKnowledgeBuildArtifact(input: {
  userId: number;
  buildId: string;
  generation: number;
  kind: KnowledgeBuildArtifactKind;
...
async function removeStagedKnowledgeBuildArtifact(input: {
  userId: number;
  buildId: string;
  generation: number;
  kind: KnowledgeBuildArtifactKind;
 ...
 */
async function listStaleKnowledgeBuildArtifactCandidates(input: {
  olderThan: Date;
  limit?: number;
}) {
  const root = storageRoot();
...
return { knowledgeBuildArtifactStorageKey, knowledgeBuildArtifactLocalPackageStorageKey, knowledgeBuildArtifactCandidateStorageKey, knowledgeBuildArtifactStorageKeyBelongsTo, persistKnowledgeBuildArtifact, stageKnowledgeBuildArtifact, readKnowledgeBuildArtifact, removeKnowledgeBuildArtifact, removeStagedKnowledgeBuildArtifact, listStaleKnowledgeBuildArtifactCandidates };
}
