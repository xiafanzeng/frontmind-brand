...
export interface KnowledgeStorageCore  {knowledgeBaseBuilds:BrandSchema["knowledgeBaseBuilds"]; getDb():Promise<MySql2Database<any>|null>;enterpriseOwnerPredicate(table:any,userId:number):SQL;}
let enterpriseOwnerPredicate: KnowledgeStorageCore["enterpriseOwnerPredicate"];
let knowledgeBaseBuilds: KnowledgeStorageCore["knowledgeBaseBuilds"];
let getDb: KnowledgeStorageCore["getDb"];
export function configureKnowledgeOpenRecovery(core: KnowledgeStorageCore) { ({ enterpriseOwnerPredicate, knowledgeBaseBuilds, getDb } = core); }
