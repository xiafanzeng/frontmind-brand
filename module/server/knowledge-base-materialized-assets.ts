...
type ActiveWorkingSetBuild = Pick<
  ...
  | "executionMode"
>;
export interface KnowledgeMaterializedAssetsCore extends Pick<ReturnType<typeof createKnowledgeBaseLocalSourceStore>,"readKnowledgeBaseLocalSource">, Pick<ReturnType<typeof createKnowledgeBasePublicResources>,"knowledgeBasePublicResource"|"knowledgeBasePublicResourceHandleMatches"> { knowledgeBaseWorkingSets:BrandSchema["knowledgeBaseWorkingSets"]; }
export function createKnowledgeBaseMaterializedAssets(core: KnowledgeMaterializedAssetsCore) {
const { knowledgeBaseWorkingSets, readKnowledgeBaseLocalSource, knowledgeBasePublicResource, knowledgeBasePublicResourceHandleMatches } = core;
...
function knowledgeBaseWorkingSetAssetUrl(input: {
  buildId: string;
...
function knowledgeBaseWorkingSetEvidenceUrl(input: {
  buildId: string;
  leafId: string;
  path: string;
  sha256: string;
...
function knowledgeBaseWorkingSetAssetInternalIdentity(input: {
  leafId: string;
...
/** Library membership authorizes a preview even before an image is assigned to a node. */
function knowledgeBaseWorkingSetLibraryAssetInternalIdentity(
  asset: Pick<KnowledgeBaseWorkingSetAsset, "assetId" | "sha256" | "path">,
) {
  return `library\0${asset.assetId}\0${asset.path}\0${asset.sha256}`;
}

function projectKnowledgeBaseWorkingSetLeafResources(input: {
  buildId: string;
  leafId: string;
...
function resolveKnowledgeBaseWorkingSetResourceByOpaqueHandle(input: {
  suppliedHandle: string;
  buildId: string;
  workingSet: ValidatedKnowledgeBaseWorkingSet;
}) {
  for (const asset of input.workingSet.manifest.assets) {
    if (knowledgeBasePublicResourceHandleMatches({
      suppliedHandle: input.suppliedHandle,
      buildId: input.buildId,
      kind: "working_set_asset",
      internalIdentity: knowledgeBaseWorkingSetLibraryAssetInternalIdentity(asset),
...
function knowledgeBaseWorkingSetLeafLocalUrls(input: {
  buildId: string;
  leafId: string;
  workingSet: ValidatedKnowledgeBaseWorkingSet;
}) {
 ...
 */
async function readValidatedActiveKnowledgeBaseWorkingSet(input: {
  db: any;
...
function resolveKnowledgeBaseWorkingSetResource(input: {
  buildId: string;
  workingSet: ValidatedKnowledgeBaseWorkingSet;
  kind: "asset" | "evidence";
  assetId?: string;
...
return { knowledgeBaseWorkingSetAssetUrl, knowledgeBaseWorkingSetEvidenceUrl, knowledgeBaseWorkingSetAssetInternalIdentity, knowledgeBaseWorkingSetLibraryAssetInternalIdentity, projectKnowledgeBaseWorkingSetLeafResources, resolveKnowledgeBaseWorkingSetResourceByOpaqueHandle, knowledgeBaseWorkingSetLeafLocalUrls, readValidatedActiveKnowledgeBaseWorkingSet, resolveKnowledgeBaseWorkingSetResource };
}
