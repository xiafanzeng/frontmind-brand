import type {KnowledgeBaseBuild,KnowledgeBaseBuildNode,KnowledgeBaseSnapshot} from "@frontmind/module-brand/schema";
import { createHash } from "node:crypto";

import { eq } from "drizzle-orm";
import JSZip from "jszip";
import { containsPrivateProviderBrand } from "./frontmind-public-brand.js";
import {
  isDashboardOwnedKnowledgePackageBuild,
  readDashboardOwnedKnowledgePackage,
} from "./knowledge-base-local-package.js";
import {
  knowledgeBasePackageWriterTaskId,
  knowledgeBasePublicationBindingHash,
} from "./knowledge-base-publication-binding.js";
...
export interface KnowledgeStorageCore extends Pick<ReturnType<typeof createKnowledgeBuildArtifactStore>,"knowledgeBuildArtifactLocalPackageStorageKey"> {knowledgeBaseBuildNodes:BrandSchema["knowledgeBaseBuildNodes"];knowledgeBaseBuilds:BrandSchema["knowledgeBaseBuilds"]; getDb():Promise<MySql2Database<any>|null>;}
let knowledgeBaseBuildNodes: KnowledgeStorageCore["knowledgeBaseBuildNodes"];
let knowledgeBaseBuilds: KnowledgeStorageCore["knowledgeBaseBuilds"];
let getDb: KnowledgeStorageCore["getDb"];
let knowledgeBuildArtifactLocalPackageStorageKey: KnowledgeStorageCore["knowledgeBuildArtifactLocalPackageStorageKey"];
export function configureKnowledgeSnapshotDownloads(core: KnowledgeStorageCore) { ({ knowledgeBaseBuildNodes, knowledgeBaseBuilds, getDb, knowledgeBuildArtifactLocalPackageStorageKey } = core); }
