import {KnowledgeBaseUploadEvidenceError} from "@frontmind/module-brand/server/knowledge-base-upload-evidence-store";
import type { ConversationTurn } from "@frontmind/module-brand/contracts/core-records";
import type { KnowledgeAsset } from "@frontmind/module-brand/contracts/dashboard-types";
...
} from "../contracts/knowledge-base-attachment.js";
import { and, asc, eq } from "drizzle-orm";
import sharp from "sharp";
import { createHash } from "node:crypto";
import { KnowledgeBasePackageBindingError } from "./knowledge-base-package-validation.js";
import type { KnowledgeBaseOfficialLogoProvenance } from "../contracts/knowledge-base-progress.js";
import { KnowledgeBaseLocalSourceError } from "@frontmind/module-brand/server/knowledge-base-local-source-store";
...
import type { createKnowledgeBaseUploadEvidenceStore } from "./knowledge-base-upload-evidence-store.js";
export interface KnowledgeCustomerUploadCore extends Pick<ReturnType<typeof createKnowledgeBaseLocalSourceStore>,"readKnowledgeBaseLocalSource">, Pick<ReturnType<typeof createKnowledgeBasePublicResources>,"knowledgeBasePublicResource">, Pick<ReturnType<typeof createKnowledgeBaseUploadEvidenceStore>, "persistKnowledgeBaseUploadEvidence"|"readKnowledgeBasePersistedCustomerUploadBytes"|"readReusableKnowledgeBaseUploadEvidenceBytes"|"readKnowledgeBaseUploadEvidence"> {
 conversationTurns: CoreSqlTable<ConversationTurn>; enterpriseOwnerPredicate(table:any,userId:number):SQL; getDb():Promise<MySql2Database<any>|null>; readStoredPresalesFile(id:string):Promise<{filename:string;sizeBytes:number;sha256:string|null;createReadStream():Readable}|null>;
}
let enterpriseOwnerPredicate: KnowledgeCustomerUploadCore["enterpriseOwnerPredicate"];
let conversationTurns: KnowledgeCustomerUploadCore["conversationTurns"];
let getDb: KnowledgeCustomerUploadCore["getDb"];
let readStoredPresalesFile: KnowledgeCustomerUploadCore["readStoredPresalesFile"];
let readKnowledgeBaseLocalSource: KnowledgeCustomerUploadCore["readKnowledgeBaseLocalSource"];
let knowledgeBasePublicResource: KnowledgeCustomerUploadCore["knowledgeBasePublicResource"];
let persistKnowledgeBaseUploadEvidence: KnowledgeCustomerUploadCore["persistKnowledgeBaseUploadEvidence"];
let readKnowledgeBasePersistedCustomerUploadBytes: KnowledgeCustomerUploadCore["readKnowledgeBasePersistedCustomerUploadBytes"];
let readReusableKnowledgeBaseUploadEvidenceBytes: KnowledgeCustomerUploadCore["readReusableKnowledgeBaseUploadEvidenceBytes"];
let readKnowledgeBaseUploadEvidence: KnowledgeCustomerUploadCore["readKnowledgeBaseUploadEvidence"];
export function configureKnowledgeBaseCustomerUploads(core: KnowledgeCustomerUploadCore) { ({ enterpriseOwnerPredicate, conversationTurns, getDb, readStoredPresalesFile, readKnowledgeBaseLocalSource, knowledgeBasePublicResource, persistKnowledgeBaseUploadEvidence, readKnowledgeBasePersistedCustomerUploadBytes, readReusableKnowledgeBaseUploadEvidenceBytes, readKnowledgeBaseUploadEvidence } = core); }
...
export function knowledgeBaseCustomerUploadImagesFromTurn(
  ...
  return customerUploadImagesFromFrozenTurn(turn, "node");
}

function customerUploadImagesFromFrozenTurn(
  turn: KnowledgeBaseCustomerUploadTurn,
  scope: "node" | "initial",
): KnowledgeBaseCustomerUploadImage[] {
  const metadata = record(turn.metadata) || {};
  ...
  const leafId = String(turn.expectedLeafId || "").trim();
  if (
    (scope === "node" ? turn.status !== "completed" : !isFrozenInitialUploadTurn(turn)) ||
    recovery?.capturedClientAttachments !== true ||
    metadata.attachmentsFrozen !== true ||
    (scope === "node" && !leafId) ||
  ...
  const officialLogoUpload = scope === "node" ? knowledgeBaseOfficialLogoUploadFromTurn(turn) : null;
...
function assertKnowledgeBaseCustomerUploadLedgerComplete(
  ...
  scope: "node" | "initial" = "node",
) {
  if (
    scope === "node" && (turn.status !== "completed" ||
...
function isFrozenInitialUploadTurn(turn: KnowledgeBaseCustomerUploadTurn) {
  return turn.operationType === "start" && ["queued", "running"].includes(turn.status) && record(turn.metadata)?.attachmentsFrozen === true;
}

/** Initial activation owns this one frozen start turn; no node or history is inferred. */
export async function verifiedKnowledgeBaseInitialUploadImagesFromTurn(
  turn: KnowledgeBaseCustomerUploadTurn,
) {
  const count = Number(record(turn.metadata)?.userAttachmentCount ?? 0);
  const recovery = record(record(turn.metadata)?.recovery);
  if (turn.operationType !== "start" || !["queued", "running"].includes(turn.status)) {
    throw new KnowledgeBasePackageBindingError("初始图片账本不属于当前创建任务");
  }
  if (count === 0 && (!Array.isArray(recovery?.attachmentManifest) || !recovery.attachmentManifest.length) && (!Array.isArray(recovery?.attachments) || !recovery.attachments.length)) return [];
  if (!isFrozenInitialUploadTurn(turn)) throw new KnowledgeBasePackageBindingError("初始图片账本尚未冻结");
  assertKnowledgeBaseCustomerUploadLedgerComplete(turn, "initial");
  const candidates = customerUploadImagesFromFrozenTurn(turn, "initial");
  const images: Array<Omit<KnowledgeBaseCustomerUploadImage, "leafId">> = [];
  for (const { leafId: _leafId, ...image } of candidates) {
    await readVerifiedKnowledgeBaseCustomerUploadImageBytes({
      image,
      sourceScope: { userId: turn.userId!, buildId: turn.buildId!, generation: turn.buildGeneration! },
    });
    images.push(image);
  }
  return images;
...
/** Keep native browser formats; safely rasterize other accepted upload formats. */
export async function prepareKnowledgeBaseInitialUploadImage(input: {
  bytes: Buffer;
  mimeType: string;
  filename: string;
}) {
  const sourceIsSvg = isPotentialCustomerSvg(input.bytes, input.mimeType, input.filename);
  if (sourceIsSvg) {
    if (input.bytes.length > CUSTOMER_UPLOAD_SVG_MAX_BYTES) throw new KnowledgeBasePackageBindingError("客户 SVG 超过 10 MB 安全预览上限");
    assertPassiveSvgForPackage(input.bytes);
  }
  const decoder = sharp(input.bytes, { failOn: "error", limitInputPixels: 100_000_000 });
  const metadata = await decoder.metadata();
  if (!metadata.width || !metadata.height) throw new KnowledgeBasePackageBindingError("初始上传图片无法解码");
  const formats = { png: "image/png", jpeg: "image/jpeg", webp: "image/webp", gif: "image/gif", avif: "image/avif" } as const;
  const nativeMime = formats[metadata.format as keyof typeof formats];
  if (nativeMime && nativeMime === input.mimeType && metadata.width * metadata.height <= 40_000_000 && input.bytes.length <= KNOWLEDGE_BASE_WORKING_SET_POLICY.archive.maxAssetBytes) {
    await decoder.stats();
    return { bytes: input.bytes, mimeType: nativeMime, width: metadata.width, height: metadata.height };
  }
  const normalized = decoder.rotate().resize({ width: 4096, height: 4096, fit: "inside", withoutEnlargement: true });
  const output = await normalized.clone().png().toBuffer({ resolveWithObject: true });
  if (output.data.length <= KNOWLEDGE_BASE_WORKING_SET_POLICY.archive.maxAssetBytes) return { bytes: output.data, mimeType: "image/png" as const, width: output.info.width, height: output.info.height };
  const compact = await normalized.webp({ quality: 95 }).toBuffer({ resolveWithObject: true });
  return { bytes: compact.data, mimeType: "image/webp" as const, width: compact.info.width, height: compact.info.height };
}
...
export function knowledgeBaseExpectedCustomerUploadsFromTurns(
  ...
  turns.forEach((turn) => assertKnowledgeBaseCustomerUploadLedgerComplete(turn));
