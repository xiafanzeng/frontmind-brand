...
} from "../../contracts/knowledge-node-workspace";
...
import {
  ...
  SheetDescription,
  SheetTitle,
} from "@frontmind/module-ui/components/ui/sheet";
...
export interface KnowledgeNodeWorkspaceProps {
  ...
  loading?: boolean;
  className?: string;
  /** Workbench nodes open in a modal; other embedded surfaces can use a drawer or inline details. */
  detailPresentation?: "drawer" | "inline" | "dialog";
  detailContainer?: HTMLElement | null;
  nodeConversation?: { leafId: string; content: React.ReactNode };
  nodeConversationDirty?: boolean;
...
function KnowledgeNodeWorkspaceSession({
  ...
  const sequentialReview =
    ...
    (progress?.workbench?.phase === "initial" ||
      ...
      leaves.some(
        (leaf) => !["confirmed", "direct_prefilled"].includes(leaf.status),
      ));
  ...
  const modalDetails = detailPresentation === "dialog";
  ...
  const selectLeaf = (leafId: string) => {
    ...
    if (dirty || nodeConversationDirty) {
  ...
  const DetailTitle = inlineDetails
    ? "h2"
    : modalDetails
      ? DialogTitle
      : SheetTitle;
  const DetailDescription = inlineDetails
    ? "p"
    : modalDetails
      ? DialogDescription
      : SheetDescription;
          ...
          <>
            ...
            {editorPreview ? (
              <div className="knowledge-node-workspace__markdown knowledge-node-workspace__draft-preview">
                <MarkdownRenderer content={draft} hideRelativeImages />
              </div>
            ) : (
              <Textarea
                ref={editorElement}
            ...
            {currentDetails && (
              <>
                ...
                {canReverifyNode && (
                  <>
                    ...
                    {selectError && (
                      <p
                        role="alert"
                        className="knowledge-node-workspace__notice"
                      >
                        {selectError}
                      </p>
                    )}
                ...
                <KnowledgeNodeLocalActions
                  ...
                  leafId={currentDetails.node.leafId}
                  leafTitle={currentDetails.node.title}
                  ...
                  onEditTargetSelected={() => {
                    ...
                    if (!detailContainer && !modalDetails) {
    ...
    >
      ...
      {leaves.length === 0 ? (
        ...
        <>
                                ...
                                return (
                                  <li key={leaf.id}>
                                    <button
                                      ...
                                      disabled={
                                        mutationPending || sequentialReview
                                      }
                                      data-review-locked={
                                        sequentialReview || undefined
                                      }
                                    >
          ...
          {inlineDetails ? (
            ...
            >
              <DialogContent
                className="knowledge-node-workspace knowledge-node-workspace-modal"
      ...
      >
        ...
        >
          <DialogHeader>
            <DialogTitle>
              {nodeConversationDirty && !dirty
                ? "当前节点有未发送内容"
                : "当前节点有未保存修改"}
            </DialogTitle>
            <DialogDescription>
              {nodeConversationDirty && !dirty
                ? "输入和附件会保留为当前节点草稿，下次打开 AI 修改时可以继续。"
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={mutationPending}
            >
              {nodeConversationDirty && !dirty ? "保留草稿并继续" : "放弃修改"}
            </Button>
            {dirty && (
              <Button
                type="button"
