...
import KnowledgeImageLibrary, { PreviewKnowledgeImageLibrary } from "./KnowledgeImageLibrary";
import {
  type CSSProperties,
  createContext,
  useContext,
...
import { useRuntimeContext } from "../host";
import { projectResourceUrl } from "../host";
...
import { getUnsavedWorkspaceDrafts } from "../host";
...
import { Home } from "../host";
import { setWorkbenchTaskQuery } from "../conversation";
import { AgentWorkbenchShell } from "../host";
import type {
  KnowledgeBaseLeafStatus,
  KnowledgeBaseProgressDto,
} from "../../contracts/knowledge-base-public-progress";
...
export default function EmbeddedKnowledgeBasePanel({
  ...
  const [standaloneImagesBusy, setStandaloneImagesBusy] = useState(false);
  const [standaloneImagesDirty, setStandaloneImagesDirty] = useState(false);
  ...
  const knowledgeQuery = trpc.workspace.knowledge.useQuery(undefined, {
    enabled: !previewMode && runtime.enabled,
    retry: false,
  ...
  useEffect(() => {
    ...
    if (resetNeedsFreshConversation) {
      ...
      trpcUtils.workspace.knowledge.clear();
  ...
  const knowledgeDownload =
    ...
    ) : displayedSnapshot || knowledgeUpdating ? (
      <Button
        variant="operatorOutline"
        disabled
        title={knowledgeUpdating ? "知识库发布中" : "知识库归档暂不可下载"}
      >
        <Download className="h-4 w-4" />
        下载知识库 ZIP
      </Button>
    ...
    <div className="knowledge-workspace-actions">
      {(unified || page === "build") && !previewMode && (
        <ManualKnowledgeUpdateButton
          disabled={editingBlocked || standaloneImagesBusy || standaloneImagesDirty}
          onPendingChange={setKnowledgeUpdating}
          onUpdated={async () => {
            await Promise.all([
              knowledgeQuery.refetch(),
              trpcUtils.workspace.portal.invalidate(),
              trpcUtils.workspace.dashboard.invalidate(),
              trpcUtils.workspace.brandQuestionUniverse.observe.invalidate(),
            ]);
      ...
      {!workbench && !previewMode && displayedSnapshot && (
        <KnowledgeImageLibrary
          projectId={projectId}
          resetRevision={resetQuery.data?.revision}
          disabled={editingBlocked || knowledgeUpdating}
          onBusyChange={setStandaloneImagesBusy}
          onDirtyChange={setStandaloneImagesDirty}
        />
      )}
      {!previewMode && resetQuery.data && (
        <KnowledgeResetButton
          disabled={editingBlocked || knowledgeUpdating || standaloneImagesBusy || standaloneImagesDirty}
      ...
      >
        {!unified && page === "display" ? (
          ...
          <PreviewBuildFlow
            ...
            snapshot={displayedSnapshot ?? undefined}
  ...
  return (
    <>
      ...
      >
        <DialogContent
          style={
            { "--module-accent": "#16794f" } as CSSProperties
          }
...
export function knowledgePublicationPollingInterval(
  progress?: KnowledgeBaseProgressDto | null,
) {
  return progress?.build.hasPublishedSnapshot === false &&
...
function ManualKnowledgeUpdateButton({
  ...
  const [publicationTimedOut, setPublicationTimedOut] = useState(false);
  const progressQuery = trpc.workspace.knowledgeProgress.useQuery(
      ...
      : undefined,
    {
      ...
      retry: false,
      refetchInterval: (query) => {
        const interval = knowledgePublicationPollingInterval(
  ...
  const autoPublishing =
    firstPublication && knowledgePublicationPollingInterval(progress) !== false;
  useEffect(() => {
    setPublicationTimedOut(false);
    if (!autoPublishing) return;
    const timer = window.setTimeout(
      () => setPublicationTimedOut(true),
      180_000,
    );
    return () => window.clearTimeout(timer);
  }, [autoPublishing, progress?.build.id, progress?.build.revision]);
  useEffect(() => {
    onPendingChange(updating || (autoPublishing && !publicationTimedOut));
    return () => onPendingChange(false);
  }, [updating, autoPublishing, publicationTimedOut, onPendingChange]);
  const observedPublication = useRef<string | null>(null);
  useEffect(() => {
    if (!progress || progress.build.status !== "published") return;
    const key = `${progress.build.id}:${progress.build.revision}:${progress.build.contentVersion}`;
    if (observedPublication.current === key) return;
    observedPublication.current = key;
    void onUpdated().catch(() =>
      toast.info("知识库已发布，资料列表暂未刷新，请重新读取页面"),
    );
  ...
  if (autoPublishing && publicationTimedOut) {
    return (
      <Button
        variant="operatorOutline"
        disabled={updating}
        onClick={async () => {
          ...
          setUpdating(true);
          try {
            const result = await withKnowledgeReadDeadline(
            ...
            if (result.data?.progress?.build.status !== "published")
              toast.info(
                "首次发布结果待核实，可继续重新读取，或重置知识库后重新创建任务",
              );
          } catch {
            if (!operation.signal.aborted)
              toast.error("暂时无法读取发布结果，请稍后重试");
          } finally {
            if (!operation.signal.aborted) setUpdating(false);
          }
        }}
      >
        {updating ? "正在读取…" : "发布待核实 · 重新读取"}
      </Button>
    );
  }
  ...
  if ((generating || autoPublishing) && !updating) {
    return (
      <Button variant="operator" disabled>
        {firstPublication ? "首次发布中…" : "生成并更新中…"}
      </Button>
    );
  }
  if (!(progress?.updateAllowed ?? progress?.packageAllowed)) {
    ...
    <>
      <Button
        ...
        onClick={() => {
          ...
          if (firstPublication) void updateKnowledgeBase();
...
function RealBuildFlow({
  ...
  const [mainComposerDirty, setMainComposerDirty] = useState(false);
  const [nodeComposerDirty, setNodeComposerDirty] = useState(false);
  const composerDirty = mainComposerDirty || nodeComposerDirty;
  ...
  useEffect(() => {
    onEditingBlockedChange(nodeDirty || nodePending || composerDirty || imagesBusy || imagesDirty);
    return () => onEditingBlockedChange(false);
  }, [nodeDirty, nodePending, composerDirty, imagesBusy, imagesDirty, onEditingBlockedChange]);
  const trpcUtils = trpc.useUtils();
  ...
  useEffect(() => {
    ...
    setMainComposerDirty(false);
    setNodeComposerDirty(false);
  }, [conversationId]);
  ...
  useEffect(() => {
    if (
      ...
      setEditTarget(null);
  }, [displayedProgress, editTarget, workbench]);
  ...
  const publicExecutionRunning = [
    ...
    "researching",
    "normalizing",
  ].includes(executionState?.runPhase ?? "");
  const hasExecutionTimeline = Boolean(
    (displayedConversation?.execution ?? displayedProgress?.execution)?.timeline
      .length,
  );
  const collaboration = (
    <>
      ...
      {workbench && (
        <KnowledgeWorkbenchActions
          ...
          disabled={updating || nodeDirty || nodePending || composerDirty || imagesBusy || imagesDirty}
      ...
      <div className="knowledge-workspace-task">
        {missingCurrentConversation || snapshotOnly ? (
          ...
          <Home
            ...
            onComposerDirtyChange={setMainComposerDirty}
          ...
          ? {
              ...
              content: (
                <KnowledgeNodeConversation
                  ...
                  progress={displayedProgress}
                  resetRevision={resetRevision}
                  disabled={updating || nodeDirty || nodePending || imagesBusy || imagesDirty}
                  onDirtyChange={setNodeComposerDirty}
                />
              ),
            }
          : undefined
      }
      nodeConversationDirty={nodeComposerDirty}
      onDetailsOpenChange={setNodeDetailsOpen}
      onNodeOpen={workbench ? undefined : focusNode}
        ...
        displayedProgress &&
          displayedConversation?.messages?.some(
            (message) =>
              ...
              message.knowledgeBase.generation ===
  ...
  return (
    <KnowledgeWorkspaceSurfaces
      ...
      resourceActions={
        workbench && (knowledgeActions.download || displayedProgress?.build.contentVersion) ? (
          <section
            className="knowledge-workbench-stage knowledge-workbench-stage--resource"
            aria-label="知识库下载"
          >
            <div className="knowledge-workbench-stage__actions">
              {knowledgeActions.download}
              {Boolean(displayedProgress?.build.contentVersion) && displayedProgress && (
                <KnowledgeImageLibrary
                  projectId={projectId}
                  conversationId={displayedProgress.build.conversationId}
                  generation={displayedProgress.workbench?.generation}
                  resetRevision={resetRevision}
                  disabled={updating || nodeDirty || nodePending || composerDirty || missingCurrentConversation}
                  onBusyChange={setImagesBusy}
                  onDirtyChange={setImagesDirty}
                  onImagesSaved={async () => {
                    await Promise.allSettled([progressQuery.refetch(), latestProgressQuery.refetch()]);
                  }}
                />
              )}
            </div>
        ...
        content:
          target === "needs_verification"
            ...
            : nextLeaf
              ...
              : "所有节点均已逐项处理，首轮知识库将自动发布。后续修改确认后可点击“更新知识库”启用新版本。",
      },
    ]);
    setDraft("");
  };
        ...
        workbench ? (
          ...
          >
            <div className="knowledge-workbench-stage__actions">
              ...
              <PreviewKnowledgeImageLibrary images={snapshot?.assets.filter((asset) => asset.mimeType.startsWith("image/") && Boolean(asset.url)).map((asset) => ({
                id: asset.id ?? asset.key,
                url: asset.url!,
                caption: asset.caption || asset.alt || asset.path,
                removable: true,
                nodes: (asset.documentIds ?? []).map((leafId) => ({ leafId, title: progress.branches.flatMap((branch) => branch.leaves).find((leaf) => leaf.id === leafId)?.title ?? leafId })),
              }))}/>
            </div>
          </section>
        ) : undefined
