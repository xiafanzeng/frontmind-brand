...
import type { KnowledgeBaseInteractionDto } from "../../contracts/knowledge-base-public-progress";
import { stripKnowledgeBaseProtocolPayloads } from "../../contracts/knowledge-base-output";
import { sanitizeFrontMindPublicText } from "../../server/frontmind-public-brand";
import { userFacingErrorMessage } from "./user-facing-error";
import { showAiBillingAction } from "../host";
import { assertChatAttachmentSizes, normalizedKnowledgeBaseUploadFilename, normalizedKnowledgeBaseUploadMimeType } from "./attachment-files";
import { knowledgeBaseLocalUploadHeaders, type KnowledgeBaseLocalUploadCoordinate } from "../../contracts/knowledge-base-local-upload";
import { siteOpsComposerLocalUploadHeaders, type SiteOpsComposerLocalUploadCoordinate } from "../../contracts/siteops-composer-local-upload";
import { dispatchKnowledgeBaseProgressUpdated, knowledgeBaseObservationFromPayload, type KnowledgeBaseObservationDto } from "./knowledge-progress";
...
export interface TaskResponse {
  ...
  object?: string;
  /** `cancelled` ends an intentional user stop, without declaring success. */
  status: "running" | "pending" | "completed" | "cancelled" | "error" | "failed";
  model?: string;
  created_at?: string;
  metadata?: {
    credit_usage?: string;
...
export interface OutputMessage {
  enterprise_qa_answer?: EnterpriseQaAnswerMeta;
  id?: string;
...
export function localAssetUploadWasCancelled(
  error: unknown,
...
export const KNOWLEDGE_BASE_TURN_REQUEST_MAX_ATTEMPTS = 3;

export const KNOWLEDGE_BASE_REQUEST_MAX_RETRY_DELAY_MS = 10_000;

export function boundedKnowledgeBaseFetch(rest: ReturnType<typeof captureWorkspaceRestOperation>, url: string, init: RequestInit, timeoutMs = 6 * 60_000) {
...
export function isTransientKnowledgeBaseRequestError(error: unknown) {
...
export function knowledgeBaseRequestRetryDelay(error: unknown, attempt: number) {
...
export async function waitForKnowledgeBaseRequestRetry(
  error: unknown,
  attempt: number,
  signal?: AbortSignal,
) {
...
export async function knowledgeBaseRequestError(
  response: Response,
  fallback: string,
...
export function assertKnowledgeBaseTurnAttachmentCoordinate(
...
export async function readUploadStatusResponse(response: Response, coordinate: Omit<KnowledgeBaseTurnAttachmentCoordinate, "turnId"> & { turnId?: string }): Promise<KnowledgeBaseUploadStatus> {
...
export type ManagedIntentCreateResponse = {
...
export function managedIntentOperationId(file: File, options: UploadFileOptions) {
  const itemId = String(options.itemId || "").trim();
...
export async function createManagedIntent(
  file: File,
  options: UploadFileOptions,
...
export const MANAGED_UPLOAD_DISCOVERY_STATES = new Set([
  "awaiting_browser",
  "receiving",
  "sealed",
  "processing",
...
export const MANAGED_UPLOAD_DISCOVERY_PHASES = new Set([
  "receiving",
  "sealed",
  "creating_provider",
  "uploading_provider",
...
export function invalidManagedUploadDiscovery(): never {
  throw new Error("Dashboard 上传恢复预约响应无效");
}

export function parseManagedUploadDiscoveryManifest(
  input: unknown,
...
export function parseManagedUploadDiscoveryReceipt(
  input: unknown,
...
export function parseManagedUploadDiscoveryPayload(input: {
  uploads?: unknown;
  reservation?: unknown;
...
export function managedUploadHandleIdentity(handle: ManagedUploadHandle) {
  return handle.intentId || handle.fileId || handle.itemId || "";
}

export function managedUploadErrorIdentity(handle: ManagedUploadHandle) {
...
export function managedIntentPhaseStage(
...
export function managedIntentPhaseProvesLocalSeal(
  phase: ManagedUploadProcessing["phase"],
) {
  return Boolean(phase && phase !== "receiving");
}

export type ManagedUploadFileIdentity = Pick<File, "name" | "size" | "type">;

export function parseManagedIntentStatus(
  input: unknown,
...
export async function recoverManagedIntent(
  handle: ManagedUploadHandle,
  file: ManagedUploadFileIdentity,
...
export function managedUploadHandleFromRecord(
  record: FileRecord,
...
export function uploadRecoveryAction(
  value: unknown,
...
export function normalizedManagedRecoveryDirective(
...
export const MANAGED_UPLOAD_INTENT_ERROR_COPY: Readonly<Record<string, string>> = {
...
export function managedUploadIntentSafeCode(code: string) {
...
export function managedUploadIntentErrorMessage(
  code: string,
  status: number,
  fallback: string,
) {
...
export async function managedUploadResponseError(
  response: Response,
  fallback: string,
...
export async function waitForManagedUploadReady(
  handle: ManagedUploadHandle,
  file: File,
  initial:
    | ManagedUploadProcessing
...
export const MANAGED_UPLOAD_RETRY_MIN_MS = 500;

export const MANAGED_UPLOAD_RETRY_MAX_MS = 5_000;

export const MANAGED_UPLOAD_BUSY_RETRY_MS = 3_000;

export function managedUploadBusyWaitState(
  error: unknown,
...
export function installFileUploadWatchdog(
  xhr: XMLHttpRequest,
  onProgress?: (percent: number) => void,
  input: {
    totalBytes: number;
...
export function uploadManagedIntentBody(input: {
  handle: ManagedUploadHandle;
  file: File;
  onProgress?: (percent: number) => void;
  signal?: AbortSignal;
...
export async function waitForManagedIntentReady(
  handle: ManagedUploadHandle,
  file: File,
...
export async function uploadFileWithManagedIntent(
  file: File,
...
export type FileUploadErrorIdentity =
  | string
  | { fileId?: string; intentId?: string }
  | undefined;

export function fileUploadErrorIdentityFields(identity: FileUploadErrorIdentity) {
  return typeof identity === "string" ? { fileId: identity } : (identity ?? {});
}

export function cancelledFileUploadError(
...
export function timedOutFileUploadError(
  identity?: FileUploadErrorIdentity,
  code:
    | "UPLOAD_BROWSER_STALLED"
    | "UPLOAD_SERVER_RESPONSE_TIMEOUT"
...
export function proxyUploadFallback(status: number) {
...
export function proxyUploadError(
  xhr: XMLHttpRequest,
  identity: FileUploadErrorIdentity,
) {
...
export function parseManagedUploadStatus(
  responseText: string,
  expectedFileId: string,
  expectedSizeBytes: number,
): ManagedUploadStatus {
...
export function parseManagedUploadStatusValue(
  input: unknown,
  expectedFileId: string,
  expectedSizeBytes: number,
): ManagedUploadStatus {
...
export function parseUploadRetentionReceiptValue(
  input: unknown,
  expectedFileId: string,
  expectedSizeBytes: number,
...
export async function uploadFileToUrlViaProxy(
