...
import { deliveryProjectHeaders } from "../host";

// Older servers ignore the preference and keep returning the legacy aliases.
const compactProgressHeaders = {
  "x-frontmind-knowledge-response": "observation-v1",
};
...
export async function fetchKnowledgeBaseProgress(
  ...
  const response = await fetch(
    ...
    { credentials: "include", headers: deliveryProjectHeaders(compactProgressHeaders) },
  );
  if (!response.ok) throw new Error(await readErrorMessage(response));
  const payload = await response.json();
  return payload?.observation?.interaction?.progress ?? payload?.progress ?? null;
}
...
export async function fetchKnowledgeBaseInteraction(
  ...
  if (!response.ok) throw new Error(await readErrorMessage(response));
  const payload = await response.json();
  return payload?.observation?.interaction ?? payload?.interaction ?? null;
}
  ...
  if (!response.ok) {
    ...
    if (response.status === 422) {
      const projectionResponse = await fetch(
        ...
        {
          credentials: "include",
          headers: deliveryProjectHeaders(compactProgressHeaders),
          signal,
        },
      );
