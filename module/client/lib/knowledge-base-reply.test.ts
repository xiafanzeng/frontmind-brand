...
vi.mock('./workspace-rest-scope',()=>({
 captureWorkspaceRestOperation:(signal:AbortSignal)=>({signal}),
 forkWorkspaceRestOperation:(parent:{signal:AbortSignal},signal:AbortSignal)=>({signal:AbortSignal.any([parent.signal,signal])}),
}));
